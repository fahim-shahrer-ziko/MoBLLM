...
def load_dataset():
    seed = 111
    random.seed(seed)
    if not os.path.exists('/kaggle/working/data4FT/temp/ft_train.json'):
        trainset, valset = [], []
        ...
        with open('/kaggle/working/data4FT/temp/ft_train.json', 'w') as fw:
            json.dump(trainset, fw, indent=4)

    df = pd.read_json('/kaggle/working/data4FT/temp/ft_train.json')
...
def train():
    ...
    config = LoraConfig(
        ...
        r=64,  # Lora 8 秩 # best maybe 32
        lora_alpha=64,  # Lora alaph 32，具体作用参见 Lora 原理 # best maybe 32
        lora_dropout=0  # Dropout 0.1 比例
    )
